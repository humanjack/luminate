...
export const videoMetadata = sqliteTable("video_metadata", {
  id: text("id").primaryKey(),
  projectId: text("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }),
  titles: text("titles", { mode: "json" }).$type<
    Array<{ text: string; ctrScore: number; reasoning: string }>
  >().notNull().default([]),
  selectedTitleIndex: integer("selected_title_index").notNull().default(0),
...
export const thumbnails = sqliteTable("thumbnails", {
  ...
  projectId: text("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }),
  preset: text("preset", {
    enum: ["bold-text", "question", "numbered-list", "reaction"],
  }).notNull(),
...
export const videoMetadataRelations = relations(videoMetadata, ({ one }) => ({
  project: one(projects, {
    fields: [videoMetadata.projectId],
    references: [projects.id],
  }),
}));
